...
func testCorrectness(t *testing.T, a netip.Addr) {
	...
	if err != nil {
		t.Fatalf("ip2location/v9 lookup error: %v", err)
	}
	if r1.Country_short == ip2locationv9_invalid_address {
		t.Fatalf("ip2location/v9 thinks a valid address is invalid...")
	}

	r2, err := IP2x_DB.Lookup(a)
	if err != nil {
		t.Fatalf("ip2x lookup error: %v", err)
	}
	...
	for x, y := range fmap {
		...
		case string:
			if exp, act := v != ip2locationv9_not_supported, IP2x_DB.Has(x); exp != act {
				t.Errorf("ip2location/v9 thinks field %s (ip2x.%s) should exist=%t (value %q), but ip2x thinks %t", y, x, exp, v, act)
			}
			if act, _ := r2.GetString(x); v != act && !(v == ip2locationv9_not_supported && act == "") {
				t.Errorf("ip2location/v9 thinks field %s (ip2x.%s) should be %#v, but ip2x thinks %#v", y, x, v, act)
			}
		case float32:
